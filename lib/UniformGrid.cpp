    ...
    : m_is_grid_valid(false), m_grid(nullptr), m_num_x_cells(0), m_num_y_cells(0), m_num_z_cells(0)
{
    ...
    for (std::shared_ptr<IRayHittable> hittable : m_objects)
    {
...
std::vector<std::shared_ptr<IRayHittable>>& UniformGrid::GetObjects()
{
    return m_objects;
}
...
void UniformGrid::Create()
{
    ...
    m_grid = new UniformGridEntry[m_num_x_cells * m_num_y_cells * m_num_z_cells];

    for (std::shared_ptr<IRayHittable> hittable : m_objects)
    {
        const AABB bounding_box = hittable->BoundingBox();
        ...
        for (int i = min_bound_grid_index.m_x; i <= max_bound_grid_index.m_x; i++)
        {
            for (int j = min_bound_grid_index.m_y; j <= max_bound_grid_index.m_y; j++)
            {
                for (int k = min_bound_grid_index.m_z; k <= max_bound_grid_index.m_z; k++)
                {
                    ...
                    m_grid[one_dimensional_index].hittables.insert(hittable);
                }
            }
        }
    }
...
Vec3 UniformGrid::DetermineCellSize() const
{
